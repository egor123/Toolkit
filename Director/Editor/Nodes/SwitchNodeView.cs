...
namespace Lostbyte.Toolkit.Director.Editor
{
    ...
    public class SwitchNodeView : PlayableTrackNodeView
    {
        ...
        private void AddElement(PlayableTrackNode node = null, float value = 0)
        {
            var element = new VisualElement() { style = { flexDirection = FlexDirection.Row } };
            var weightField = new FloatField("Time") { value = value };
        ...
        {
            (Node as SwitchNode).Nodes = _nodes.Select(n => new SerializedTuple<float, PlayableTrackNode>(n.Item1.value, n.Item2.connected ? (n.Item2.connections.First()?.input.node as PlayableTrackNodeView).Node : null)).ToList();
        }
    }
}